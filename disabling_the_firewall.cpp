...
void setup() {
  ...
  // Открытие порта 8080 в брандмауэре
  Keyboard.print("netsh advfirewall firewall add rule name=\"Open Port 8080\" dir=in action=allow protocol=TCP localport=8080");
  delay(100);
  Keyboard.press(KEY_RETURN);    // Нажатие на Enter
  delay(100);
  Keyboard.releaseAll();         // Освобождение всех клавиш
